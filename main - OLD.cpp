...
void Initialize() {
    ...
    state.player->acceleration = glm::vec3(0, 0, 0); //Setting Acceleration to Gravity
    ...
    //Initialize laser - Only one laser allowed

    state.laser = new Entity();
    state.laser->entityType = LASER;
    state.laser->textureID = LoadTexture("laser.png");
    state.laser->isActive = false;
    state.laser->position = state.player->position;
    ...
    //Starting Position
    state.underlings[0].position = glm::vec3(-4.5f, -3.25f, 0);
    state.underlings[1].position = glm::vec3(-4.5f, 3.25f, 0);
    state.underlings[2].position = glm::vec3(4.5f, -3.25f, 0);
    state.underlings[3].position = glm::vec3(4.5f, 3.25f, 0);
    ...
    //Starting Position
    state.midboss[0].position = glm::vec3(4.5f, 0, 0);
    state.midboss[1].position = glm::vec3(-4.5f, 0, 0);
    ...
    state.boss->animIndices = state.boss->animDown;
        ...
        //Starting Position
    state.boss->position = glm::vec3(0, 3.25f, 0);
}
...
void Update() {
    ...
    while (deltaTime >= FIXED_TIMESTEP) {
        ...
        state.player->Update(FIXED_TIMESTEP, state.player, state.underlings, UNDERLING_COUNT, state.midboss, MIDBOSS_COUNT, state.boss);

        for (int i = 0; i < UNDERLING_COUNT; i++) {
            state.underlings[i].Update(FIXED_TIMESTEP, state.player, state.underlings, UNDERLING_COUNT, state.midboss, MIDBOSS_COUNT, state.boss);
        }

        for (int i = 0; i < MIDBOSS_COUNT; i++) {
            state.midboss[i].Update(FIXED_TIMESTEP, state.player, state.underlings, UNDERLING_COUNT, state.midboss, MIDBOSS_COUNT, state.boss);
        }

        state.boss->Update(FIXED_TIMESTEP, state.player, state.underlings, UNDERLING_COUNT, state.midboss, MIDBOSS_COUNT, state.boss);
